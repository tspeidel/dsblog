...
Visualization pioneer Edward Tufte has a very effective visual demonstration on **streak-guessing** around time (adapted from the original). On the top figure we see the actual win-loss record.  At the bottom, notice what happens when we randomize the order on time.
...
*2009 Boston Red Sox win-loss record: when 4 or more wins occurs one after the other, the series is drawn in red. Commentaries from players' attempt to highlight a causal mechanism for each winning streak.*
...
*Not so fast: 3 different random (boostrapped) resamples from the same data. Notice how streaks occurr with nearly the same frequency.   Despite the effort, there is little evidence for a causal mechanism. The invisible hand of chance.*
